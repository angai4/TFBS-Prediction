#!/usr/bin/env python2
"""
TFFM (First-Order) cross-validation scorer

What this script does (per TF, per fold):
  1) Load a MEME motif and initialize a first-order TFFM model.
  2) Train the TFFM on the fold's training FASTA.
  3) Score the fold's positive (test) and negative (GC-matched background) FASTAs.
  4) Compute ROC and AUC, save a ROC plot, and write per-TF/per-fold AUC CSVs.
  5) Aggregate everything into a master CSV across TFs.

Expected project layout under ~/msc_project:
  data/<TF>/crossval/fold_XX/{train_TXX.fa, test_FXX.fa, bg_BXX.fa}
  results/<TF>/motif/meme.meme                    # MEME motif used to initialize TFFM
  results/<TF>/tffm_1st_order/{model,figures,metrics}  # outputs written here
  master_results/tffm_1st_order/metrics/master_auc.csv # global AUC table

Notes:
  - This script is Python 2 (as required by the TFFM library in many setups).
  - Matplotlib backend is set to Agg for headless plot saving.
  - The worker pool size is capped at max_procs (40). Change max_procs to tune.
"""
...
def process_fold(args):
    ...
    # Train initial model
    tffm = tffm_module.tffm_from_meme(meme_file, TFFM_KIND.FIRST_ORDER)
    init_xml = os.path.join(model_dir, "tffm_1st_order_initial_fold_{}.xml".format(fold_number))
    tffm.write(init_xml)
    tffm.train(train_seqs)
    trained_xml = os.path.join(model_dir, "tffm_1st_order_fold_{}.xml".format(fold_number))
    tffm.write(trained_xml)
    logging.info("%s %s: Model trained and saved to %s", tf, fold_name, trained_xml)
    del tffm

    # Load trained model
    tffm = tffm_module.tffm_from_xml(trained_xml, TFFM_KIND.FIRST_ORDER)
    ...
    plt.title('%s - %s TFFM First-Order ROC' % (tf, fold_name))
    plt.legend(loc='lower right')
    plt.grid(True)
    plt.tight_layout()
    fig_path = os.path.join(figures_dir, 'tffm_1st_order_roc_%s.png' % fold_name)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run TFFM 1st-order cross-val and ROC/AUC.")
    parser.add_argument(
    ...
    masters_root = os.path.join(project, 'master_results', 'tffm_1st_order', 'metrics')
    ...
    for tfdir in sorted(os.listdir(data_root)):
        ...
        # Prepare per-TF dirs
        tffm_root = os.path.join(results_root, tfdir, 'tffm_1st_order')
    ...
    for tf, recs in tf_groups.items():
        ...
        if not valid:
            ...
            continue
        # per-TF CSV
        csv_path = os.path.join(results_root, tf, 'tffm_1st_order', 'metrics', '%s_auc.csv' % tf)
